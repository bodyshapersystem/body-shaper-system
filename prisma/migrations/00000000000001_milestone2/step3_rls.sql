...
-- table's authUserId, which equals auth.uid()::text for the signed-in session).
CREATE POLICY "clients_select_own" ON "clients"
  FOR SELECT USING (
    "userId" IN (SELECT "id" FROM "users" WHERE "authUserId" = auth.uid()::text)
  );
...
CREATE POLICY "messages_select_own" ON "messages"
  FOR SELECT USING (
    "threadId" IN (
      ...
      WHERE c."userId" IN (SELECT "id" FROM "users" WHERE "authUserId" = auth.uid()::text)
    )
  );
