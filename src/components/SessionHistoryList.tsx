 ...
 * Alignment™ snapshot frozen at the time it was saved). Shared
 * between the Hub's client detail view and the client portal's
 * Systems & Sessions "Sessions" tab, so both show the exact same
 * real records with no duplication.
 */
export default function SessionHistoryList({ sessions }: { sessions: SessionRecord[] }) {
        ...
        return (
          <div key={s.id} className="cah-appt-row">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
              <p className="cah-appt-title">SESSION {String(sessions.length - i).padStart(2, "0")}</p>
