      ...
      <div className="doc-card-grid">
        {items.map((item) => (
          <div key={item.id} className="rw-reward-card">
            ...
            <p className="pay-history-meta">{categoryLabels[item.category] ?? item.category} · {item.creditCost} credits · {item.available ? "Available" : "Unavailable"}</p>
            {canManage && (
          ...
          <div className="bp-sheet" onClick={(e) => e.stopPropagation()}>
            ...
            <form action={handleSubmit} className="bp-sheet-form">
              ...
              <div className="bp-sheet-grid">
                ...
                <label className="sched-label">Credit Cost<input name="creditCost" type="number" defaultValue={editing !== "new" ? editing.creditCost : ""} required className="sched-select" /></label>
              </div>
